	...
	function FlashForgeViewModel(parameters) {
		...
		if (self.editor.fromProfileData !== undefined && self.editor.toProfileData !== undefined) {
			...
			self.editor.fromProfileData = function(data) {
				...
				if (data.noG91 === undefined)
					data.noG91 = false;
				if ($('#ff_noG91').length)
					$('#ff_noG91').prop('checked', data.noG91);
			};

			self.editor.toProfileData = function() {
				profile = this.origToProfileData();
				if ($('#ff_noG91').length)
					profile.noG91 = $('#ff_noG91').prop('checked');
				return profile;
			};
		}
			...
			$("#settings_printerProfiles_editDialog_axes .form-horizontal").prepend(
				'<div class="control-group">' +
					...
					'<div class="controls"><label class="checkbox">' +
						'<input id="ff_noG91" type="checkbox" data-bind="checked: noG91">' +
						'Select if movement does not work (FlashForge Finder 2, Guider 2, etc)' +
					'</label></div>' +
				'</div>');
		}
		...
		self.onEventConnected = function() {
			// disable this for now as it causes connection printer to hang if printer is already printing
			/*
			if (self.ledStatus())
				self.setLed(self.ledColor())
			else
